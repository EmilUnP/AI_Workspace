      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
          {teacherClasses.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                <Link
                  href={params.status ? `/teacher/exams?status=${params.status}` : '/teacher/exams'}
                ...
                {teacherClasses.map((c) => (
                  <Link
                    key={c.id}
                    href={`/teacher/exams?classId=${c.id}${params.status ? `&status=${params.status}` : ''}`}
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-hide">
            <Link
              href={params.classId ? `/teacher/exams?classId=${params.classId}` : '/teacher/exams'}
            ...
            <Link
              href={params.classId ? `/teacher/exams?status=published&classId=${params.classId}` : '/teacher/exams?status=published'}
            ...
            <Link
              href={params.classId ? `/teacher/exams?status=draft&classId=${params.classId}` : '/teacher/exams?status=draft'}
                ...
                <div key={exam.id} className="p-4 transition-colors hover:bg-gray-50/50">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex min-w-0 flex-1 items-center gap-3">
                      <Link href={`/teacher/exams/${exam.id}`} className="flex min-w-0 flex-1 items-center gap-3">
                  ...
                  <tr key={exam.id} className="transition-colors hover:bg-gray-50/70">
                    <td className="py-4 pl-5 pr-3 sm:pl-6">
                      <div className="flex items-center gap-3">
                        <Link href={`/teacher/exams/${exam.id}`} className="flex min-w-0 flex-1 items-center gap-3 hover:opacity-90">
                          <div className={`flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl ${exam.is_published ? 'bg-emerald-100 text-emerald-600 ring-1 ring-emerald-200/50' : 'bg-amber-100 text-amber-600 ring-1 ring-amber-200/50'}`}>
                            <FileText className="h-5 w-5" />
                          </div>
