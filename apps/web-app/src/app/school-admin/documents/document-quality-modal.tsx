...
export function DocumentQualityModal({ document, onClose, translations }: DocumentQualityModalProps) {
  ...
  const hasError = document.processing_status === 'failed' || Boolean(document.processing_error_message)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-xl border border-gray-200 bg-white" role="dialog" aria-labelledby="document-quality-title">
        <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3">
          ...
          <button type="button" onClick={onClose} className="rounded p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600" aria-label="Close">
            <X className="h-5 w-5" />
          </button>
        </div>
              ...
              <div className="mt-2 flex gap-4 text-sm">
                ...
                {document.avg_chunk_size != null && document.avg_chunk_size > 0 ? (
                  <div>
                    ...
                    <span className="font-medium text-gray-900">{Math.round(document.avg_chunk_size / 4).toLocaleString()}</span>
                  </div>
                ) : null}
              </div>
            </div>
